...
use Modules\Heroes\Config\Config;
use Modules\Heroes\Models\M_Heroes;
...
class Heroes extends BaseController{
    ...
    public function get_form($data = []){
        ...
        return view('Modules\Templates\Views\menu')
            . view('Modules\heroes\Views\form', $data);
    }
    ...
    public function load_table() {
        $M_heroes = new M_heroes();
        $page    = (int) ($this->request->getGet('page') ?? 1);
        $perPage = 10;
        $total   = (int) $M_heroes->get_count_active();

        $data = $this->get_row_table();

        $data['table_body'] = view('Modules\heroes\Views\table_body', $data);
        $data['spinner'] = view('Modules\Templates\Views\spinner');
        return view('Modules\Templates\Views\menu')
            . view ('Modules\heroes\Views\table_header', $data);
    }
    ...
    public function table_search(){
        ...
        $response = array(
            'success' => true,
            'results' => view('Modules\heroes\Views\table_body', $data),
    ...
    public function get_row_table(array $search = null) {
        ...
        return $M_heroes->getPagination($perPage, $page, $total, $search);
    }
    ...
    {
        $M_heroes = new M_heroes();
        $data['data'] = $M_heroes->get_edit($uuid);
    ...
    public function save_form()
    {
        ...
        if ($id === '') {
            $M_heroes->savedb($data);
        } else {
            $M_heroes->update($id, $data);
        }
    ...
    {
        helper(['form']);
        $config = config('Modules\heroes\Config');
    ...
    public function validateImport($importData){
        ...
        $validation->setRules(config('Modules\heroes\Config')->rules);
            ...
            foreach ($sheetData as $row) {
                ...
                if ($counter >= $this->config->str_num) {
                    ...
                    if (!is_null($row[0]) || !is_null($row[1]) || !is_null($row[2]) || !is_null($row[3]) || !is_null($row[4]) || !is_null($row[5])) {
                        ...
                        if ($this->validateImport($heroesData) && $C_Small_business->validateImport($smallBusinessData)) {
                            $smallBusinessId = $C_Small_business->save_record($smallBusinessData);
                            $heroId = $M_heroes->savedb($heroesData);
